...
# Moved init_db call outside the function to ensure it runs when the module is imported/run
# as per the __main__ block's logic. However, it's safer to ensure it's called only once.
# The @app.before_first_request decorator is deprecated in newer Flask versions.
# A common pattern is to call init_db() directly in __main__ or manage it via app context.
# For this setup, we'll rely on the __main__ block to call init_db.
...
def api_banned_ips():
    ...
    try:
        banned_ips_data = get_banned_ips()  # ต้อง return list ของ dicts: [{"ip_address": ..., "reason": ..., "ban_timestamp": ...}, ...]
...
def manage_ip():
    ...
    if not ip_address or not action:
        return jsonify({"success": False, "message": "Missing ip_address or action"}), 400

    try:
        if action == 'ban':
            if ban_ip(ip_address, reason):
                return jsonify({"success": True, "message": f"IP {ip_address} banned successfully."})
            else:
                # Could be already banned or an error
                if is_ip_banned(ip_address):
                    return jsonify({"success": False, "message": f"IP {ip_address} is already banned."}), 409
                else:
                    return jsonify({"success": False, "message": f"Error banning IP {ip_address}."}), 500
        elif action == 'unban':
            if unban_ip(ip_address):
                return jsonify({"success": True, "message": f"IP {ip_address} unbanned successfully."})
            else:
                return jsonify({"success": False, "message": f"IP {ip_address} was not found or could not be unbanned."}), 404
        else:
            return jsonify({"success": False, "message": "Invalid action specified. Use 'ban' or 'unban'."}), 400
    except Exception as e:
