/**
 * k6/baseline.js — Baseline performance test (clean traffic only)
 *
 * FIX: เพิ่ม 429 ใน expectedStatuses → rate limit ไม่นับเป็น http_req_failed
 *      เพิ่ม Counter แยก 429 / 403 / 5xx เพื่อ diagnose ได้ชัดขึ้น
 */
...
const status429   = new Counter('resp_status_429');  // FIX: แยก rate limit
...
export const options = {
    ...
    thresholds: {
        ...
        resp_status_403:   ['count<5'],    // FP ใน baseline ต้องแทบไม่มี
    },
...
export default function () {
    ...
    errorRate.add(res.status !== 200 && res.status !== 429);  // 429 ไม่ใช่ error จริง
